...
bool setup(BelaContext* context, void* userData)
{
    ...
    // Parameter set
    feedbackParameter = 0.5f; // 0...0,98
    dryWetParameter = 0.5f; // 0...1
    depthParameter = 0.5f; // 0...1
    rateParameter = 1.f; // 0,1...20
...
void process_chorus_flanger(float *inBuffer, int inWritePointer, float *outBuffer, int outWritePointer, float Fs)
{
    ...
    float lfoMapped = processFlanger(lfoOut);//(doChorus == 1) ? processChorus(lfoOut) : processFlanger(lfoOut);
    ...
    t1 = ccnt_read();
    rt_printf("\r%u cycles process #### ", t1-t0);
    ...
    for(unsigned int n = 0; n < context->audioFrames; n++) {
        if(gAudioFramesPerAnalogFrame && !(n % gAudioFramesPerAnalogFrame)) {
            if(n==0)
            {
                ...
                feedbackParameter = (float)map(analogRead(context, n/gAudioFramesPerAnalogFrame, gAnalogIn), 0, 1, 0, 198)/100.f;
